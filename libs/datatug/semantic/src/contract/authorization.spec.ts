import { decodeErrorEnvelope } from './decoders';
import { describe, expect, it } from 'vitest';
import { isAuthorizationResult, isUpdateResponse } from './authorization';
...
describe('layered denial details', () => {
  const authorization = {
    ...(executionResult(true) as Record<string, unknown>),
    allowed: false,
    result: 'deny',
  ...
  const envelope = {
    error: { code: 'ACCESS_DENIED', message: 'Access denied', requestId: 'r1' },
    details: { authorization },
  };
  it('preserves multiple owner blockers and unknown reason codes', () => {
    expect(
      decodeErrorEnvelope(envelope).details?.authorization?.blockers,
    ).toEqual(authorization.blockers);
  });
  it('rejects malformed blocker facts and contradictory allowed denials', () => {
    for (const invalid of [
      { ...authorization, blockers: [null] },
      { ...authorization, allowed: true },
    ]) {
      expect(() =>
        decodeErrorEnvelope({
          ...envelope,
          details: { authorization: invalid },
        }),
      ).toThrow();
    }
  });
