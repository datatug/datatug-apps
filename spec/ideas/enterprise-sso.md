...
How might a company adopt OIDC or SAML sign-in in DataTug.app without support tickets, duplicate user accounts, a second organisation model, or weakening Sneat's existing space membership boundary?
...
Build a product-neutral enterprise-auth extension in `sneat-core-modules/auth/sso` and mount it through narrow adapters in the existing Sneat Go host. OIDC and SAML protocol engines converge on one trust, Firebase linking, Sneat membership, routing, and browser-session handoff service. Put its reusable Angular screens and API client in `@sneat/auth-ui`, so DataTug.app is the first consumer rather than the owner of a one-off identity stack; sneat.work, sneat.team, and later Sneat applications can register the same routes. Store the source-of-truth configuration at `/spaces/{spaceID}/ext/sso`; maintain global routing indexes only for lookup.
...
- `/sso-identities/{sha256(protocol + NUL + issuer/entityID + NUL + subject/NameID)}`: global stable identity linkage to a Firebase/Sneat UID, with link-time email and associated space IDs.
- `/sso-audit/{eventID}`: sanitized security-event records scoped by `spaceID`; no tokens, assertions, secrets, raw subjects, or email addresses.
...
- Provider-neutral self-service requires a random DNS TXT challenge under the claimed domain. Entra Graph verified-domain evidence and explicitly allow-listed generic issuer/domain pairs remain alternate trust paths.
- Generic OIDC does not prove domain ownership by itself. Without DNS proof it is accepted only when the issuer/domain pair is explicitly allow-listed in server configuration; activation is rejected before discovery/network access when neither evidence path exists.
...
- OIDC discovery, token, redirect, and JWKS traffic uses a no-proxy backend client that rejects loopback, private, link-local, and reserved network destinations in production, preventing a configured generic issuer from becoming an SSRF path. Loopback HTTP is available only behind explicit local-test configuration.
...
- One OIDC or SAML provider and one primary domain per space in the UI.
- OIDC authorization code flow with discovery, backend token validation, PKCE, nonce, and state.
- SP-initiated SAML Web SSO with signed requests, strict assertion validation, and persistent NameID linkage.
- DNS TXT self-service, Entra verified-domain self-service, plus explicitly trusted generic issuers for controlled use.
...
- Automated Go tests and Playwright E2E with deterministic signed OIDC/SAML providers plus a pinned Keycloak Docker profile for real-provider OIDC interoperability.

## Not Doing (and Why)

- SCIM, group-to-role mapping, IdP-initiated SAML login, SLO, multiple IdPs, multi-domain UI, CLI/mobile-specific SSO, Conditional Access management, enterprise billing gates, and elaborate audit reporting remain later capabilities.
