...
For a follow-up request, the browser rebuilds bounded context from recent questions, model-produced DTQL, RecordSet IDs, column names, and row counts. The model can request an `In` predicate whose right side references a saved RecordSet ID and source column. The browser checks that reference against the active session, reads the saved snapshot through DALgo, and binds distinct scalar values locally before parsing and executing the query. The executed DTQL, original model action, and parent RecordSet ID are retained for provenance. Result row and cell values stay in local storage and are not copied into the AI request. This preserves the Phase 1 direct provider flow and leaves row selection, docking, and bookmarks for later phases.
