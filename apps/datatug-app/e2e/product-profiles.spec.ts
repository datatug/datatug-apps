...
// today. Screenshots land in the scratch directory named by the founder's
// brief, for a human/adversarial review pass.
const SCREENSHOT_DIR =
  '/private/tmp/claude-501/-Users-alex-projects/dcf47935-df08-4214-803e-eb3ebcf509c7/scratchpad/incidentius-scaffold';
...
test('?profile=incidentius lands on the incident list with the "Houston" entry point (AC:incidentius-profile-home)', async ({
  ...
  await page.screenshot({
    path: `${SCREENSHOT_DIR}/incidentius-home.png`,
    fullPage: true,
  });
});
...
test('the datatug profile always shows an Incidents item in the side menu, with no project open (AC:incidents-always-in-datatug-menu)', async ({
  ...
  await expect(page.getByRole('link', { name: 'DataTug.app home' })).toBeVisible();
  ...
  await page.screenshot({
    path: `${SCREENSHOT_DIR}/datatug-menu-incidents-item.png`,
    fullPage: true,
  });
